...
export type ScientificDataGridColDef = GridColDef & {
  units?: string;
  decimals?: number;
  sigFigs?: number;
  isFormula?: boolean;
...
interface ScientificDataGridProps extends Omit<DataGridProps, 'columns'> {
  columns: ScientificDataGridColDef[];
}
...
const getGridColumns = (columns: ScientificDataGridColDef[]) => {
        ...
        if (isFormula) {
          return (
            <CellWrapper hasPopover={hasPopover}>
              <ChemicalFormula content={params.value} />
            </CellWrapper>
          );
        } else {
          return (
 ...
 */
export const ScientificDataGrid: React.FC<ScientificDataGridProps> = ({
  rows,
  columns,
  ...rest
}) => {
